...
function TypeScale() {
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'var(--pluto-font-family-sans)' }}>
      ...
      <div style={{ borderLeft: '2px solid var(--pluto-color-border-default)', paddingLeft: '24px' }}>
        {scale.map((item) => (
          <div
            key={item.name}
            style={{
          ...
          >
            <div style={{ fontSize: item.size, lineHeight: 1.2, flex: 1 }}>
              The quick brown fox
            </div>
            <div style={{ minWidth: '220px', textAlign: 'right' }}>
              <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--pluto-color-text-secondary)' }}>
                {item.name}
              </div>
...
function FontFamilies() {
  ...
  return (
    <div style={{ padding: '24px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '8px', fontFamily: 'var(--pluto-font-family-sans)' }}>
        Font Families
      </h1>
      <p style={{ fontSize: '14px', color: 'var(--pluto-color-text-secondary)', marginBottom: '40px', fontFamily: 'var(--pluto-font-family-sans)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        {families.map((f) => (
          ...
          >
            <div
              style={{ fontSize: '28px', fontFamily: f.cssVar, marginBottom: '12px', lineHeight: 1.3 }}
            >
              {f.sample}
            </div>
            <div style={{ display: 'flex', gap: '24px', fontFamily: 'var(--pluto-font-family-sans)', fontSize: '12px' }}>
              <span style={{ fontWeight: 600 }}>{f.name}</span>
              <code style={{ opacity: 0.6 }}>--pluto-font-family-{f.name.toLowerCase().split(' ')[0]}</code>
