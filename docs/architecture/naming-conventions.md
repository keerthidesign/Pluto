...
**Prefix:** `--pluto-` is the global namespace. Never omit it — prevents collisions with third-party
CSS.
...
1. Add `"$extensions": { "com.pluto.deprecated": true, "com.pluto.replacement": "new.token.name" }`
   to the old token
